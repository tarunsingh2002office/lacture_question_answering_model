...
async def paths():
    try:
        ...
        await asyncio.to_thread(input_pdf_dir.mkdir, exist_ok=True, parents=True)
        await asyncio.to_thread(input_video_dir.mkdir, exist_ok=True, parents=True)
        await asyncio.to_thread(input_audio_dir.mkdir, exist_ok=True, parents=True)
        await asyncio.to_thread(input_text_dir.mkdir, exist_ok=True, parents=True)
        return all_paths
    except Exception as err:
        raise Exception(f"something went wrong {err}")
...
async def QuestionAnswerGenerationModel(request: Request, uploaded_file: List[UploadFile] = File(...), 
                                        number_of_questions: int = Form(...), hinglish: bool = Form(...)):
    try:
        ...
        for i, upload in enumerate(uploaded_file):
            # validate file type for each uploaded file
            if upload.content_type != "video/mp4":
                return JSONResponse(content={"message": f"Invalid file type for file {upload.filename}. Please upload MP4 videos only."}, status_code=400)
            file_bytes = await upload.read()
            print(f"here is {i}")
            print("-"*100)
            # create per-file target paths to avoid overwriting
            ...
            await audio_to_text(path=audio_target, text_file_path=text_file_path, hinglish=hinglish,)
            # await save_text_to_pdf(text_file_path=text_file_path, output_path=all_paths["input_pdf_file"], font_path=all_paths["font_path"])
